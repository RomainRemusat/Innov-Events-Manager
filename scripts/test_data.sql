...
CREATE TABLE prospects (
                           id INT AUTO_INCREMENT PRIMARY KEY,
                           user_id INT NULL, -- LIAISON CLIENT : Permet d'associer un devis à un compte utilisateur
                           ...
                           created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- Contrainte RGPD : Si le client supprime son compte, ses données prospects sont purgées
                           FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Injection de prospects avec des textes descriptifs et des budgets détaillés
INSERT INTO prospects (user_id, company_name, contact_name, email, phone, event_type, event_date, estimated_participants, budget, description, status)
VALUES
    (
        NULL, -- Ce prospect n'a pas encore créé de compte client
        'TechCorp',
        'Jean Dupont',
    ...
    (
        3, -- Associé au compte d'Alice (ID 3)
        'Luxury Hotel Group',
        'Marc Petit',
